...
function buildExportSVG({designId,t,sch,range,title,img}){
  ...
  const titleUpperEsc=escapeXml(rawTitle.toUpperCase());
  title=escapeXml(title);
  range=escapeXml(range);
  sch=sch.map(it=>{const rawTextLen=(it.text==null?"":String(it.text)).length;return{...it,rawTextLen,text:escapeXml(it.text),time:escapeXml(it.time)};});
  ...
  <text x="400" y="55" text-anchor="middle" fill="${t.text}" font-size="13" font-family="'Georgia',serif" letter-spacing="6" opacity="0.35">${rawTitle?titleUpperEsc:"WEEKLY SCHEDULE"}</text><text x="400" y="76" text-anchor="middle" fill="${t.accent}" font-size="11" font-family="'Georgia',serif" letter-spacing="4" opacity="0.25">${range}</text>${lin(160,90,640,90,t.accent,0.5,0.12)}${cir(160,90,2,t.accent,0.1)}${cir(640,90,2,t.accent,0.1)}${rows}
...
function ExportModal({svgString,onClose,accent,dark,title,range}){
  ...
  return(<div role="dialog" aria-modal="true" aria-label="画像を保存" style={{position:"fixed",inset:0,zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",background:bg,backdropFilter:"blur(6px)"}} onClick={onClose}><div style={{background:pBg,borderRadius:20,maxWidth:720,width:"92%",maxHeight:"92vh",overflowY:"auto",boxShadow:"0 24px 80px rgba(0,0,0,0.4)"}} onClick={e=>e.stopPropagation()}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"20px 24px 0"}}><div style={{fontWeight:900,fontSize:20,color:accent}}>📥 画像を保存</div><button onClick={onClose} aria-label="閉じる" style={{background:"none",border:"none",fontSize:24,cursor:"pointer",color:tx,opacity:.5}}>✕</button></div>
    <div style={{display:"flex",gap:8,padding:"16px 24px 0"}}>{[{id:"png",label:"🖼️ PNG"},{id:"svg",label:"✏️ SVG"}].map(x=>(<button key={x.id} onClick={()=>setTab(x.id)} aria-pressed={tab===x.id} style={{flex:1,padding:"12px",borderRadius:12,border:tab===x.id?`2px solid ${accent}`:`2px solid ${"#e2e8f0"}`,background:tab===x.id?`${accent}15`:"transparent",cursor:"pointer",fontFamily:"inherit",fontWeight:900,fontSize:15,color:tab===x.id?accent:tx}}>{x.label}</button>))}</div>
...
export default function WeeklyScheduleMaker(){
  ...
  const delT=async(i)=>{const name=myTemplates[i]?.name||"このテンプレート";if(!window.confirm(`「${name}」を削除しますか？`))return;await persist(myTemplates.filter((_,j)=>j!==i));notify("🗑️ 削除しました");};
        ...
        <div style={{width:isMobile?"100%":380,minWidth:isMobile?0:380,boxSizing:"border-box",background:pBg,borderRight:isMobile?"none":`1px solid ${bd}`,borderBottom:isMobile?`1px solid ${bd}`:"none",minHeight:isMobile?"auto":"calc(100vh - 60px)",overflowY:"auto"}}>
          <div style={{display:"flex",borderBottom:`1px solid ${bd}`}}>
            {[{id:"design",l:"🎨 デザイン"},{id:"schedule",l:"📝 予定"},{id:"templates",l:"💾 テンプレ"}].map(x=>(
              <button key={x.id} onClick={()=>setTab(x.id)} aria-pressed={tab===x.id} style={{flex:1,padding:"13px 4px",border:"none",background:tab===x.id?"white":"transparent",borderBottom:tab===x.id?`3px solid ${theme.accent}`:"3px solid transparent",color:tab===x.id?theme.accent:pTx,fontWeight:700,fontSize:13,cursor:"pointer",fontFamily:"inherit",opacity:tab===x.id?1:.5}}>{x.l}</button>
            ))}
          </div>
            ...
            {tab==="design"&&(<div>
              ...
              <div style={{background:`${theme.accent}06`,borderRadius:14,padding:14,border:`1px solid ${theme.accent}15`}}>
                ...
                <label style={{display:"flex",alignItems:"center",justifyContent:"center",height:uploadedImg?100:44,borderRadius:10,border:`1.5px dashed ${theme.accent}40`,background:`${theme.accent}05`,cursor:"pointer"}}>
                  ...
                  <input type="file" accept="image/*" aria-label="カスタム画像を選択" onChange={e=>{const f=e.target.files?.[0];if(!f)return;if(!f.type.startsWith("image/")){notify("⚠️ 画像ファイルを選択してください");return;}if(f.size>10*1024*1024){notify("⚠️ 画像が大きすぎます（10MBまで）");return;}const r=new FileReader();r.onload=ev=>setUploadedImg(ev.target.result);r.onerror=()=>notify("⚠️ 画像の読み込みに失敗しました");r.readAsDataURL(f);}} style={{display:"none"}}/>
                </label>
            ...
            {tab==="templates"&&(<div>
              ...
              {myTemplates.map((tmpl,idx)=>(<div key={idx} style={{background:iBg,borderRadius:12,padding:12,border:`1px solid ${iBd}`,marginBottom:8}}>
                ...
                <div style={{display:"flex",gap:6}}>
                  ...
                  <button onClick={()=>delT(idx)} aria-label={`テンプレート「${tmpl.name}」を削除`} style={{padding:"7px 12px",borderRadius:8,border:"1px solid #ef444440",background:"#ef44440a",color:"#ef4444",fontWeight:700,fontSize:12,cursor:"pointer",fontFamily:"inherit"}}>🗑️</button>
                </div>
              </div>))}
